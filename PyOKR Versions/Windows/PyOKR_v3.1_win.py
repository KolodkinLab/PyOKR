...
#adjust camera_framerate based on collection parameter
def Velocity_calculator(dist,b,e):
    camera_framerate = 100
    vel = camera_framerate*(dist/(abs(e-b)))
    
    return vel

#Divides velocity/stimulus speed
def Gain_calculator(vel,stim,camera_framerate):
    vel1 = vel*camera_framerate
    gain = vel1/stim
    return gain
...
def Poly_fit(lst,fd,polythresh,plotthresh,framerate):
    ...
    for x in lst:
        b, e = x
        setp = fd[b:e]
        setp_stim = stim_velocity[b:e]
        
        x1 = setp["X"].to_numpy()
        y1 = setp["epxWave"].to_numpy()
        z1 = setp["epyWave"].to_numpy()
        
        xstim = setp_stim["epxWave"].to_numpy()
        ystim = setp_stim["epyWave"].to_numpy()
        ...
        polyvals["X_vel"] = dy2 * framerate
        polyvals["Y_vel"] = dz2 * framerate
        
        polyvals["X_vel_stim"] = xstim
        polyvals["Y_vel_stim"] = ystim
        
        polyvals["X_gain"] = polyvals["X_vel"]/polyvals["X_vel_stim"]

        polyvals["Y_gain"] = polyvals["Y_vel"]/polyvals["Y_vel_stim"]
        
        polyvals[~np.isfinite(polyvals)] = np.nan
        polyvals.reset_index()
        
        vellist_for = []
        vellist_back =[]
        for x in polyvals["X_vel"]:
            if x > 0:
        ...
        gainlist_up = []
        gainlist_down =[]
        for x in polyvals["Y_gain"]:
            if x > 0:
                gainlist_up.append(x)
            elif x < 0:
                gainlist_down.append(x)

        abs_xvel = polyvals["X_vel"].abs()
        x_vel_mean = np.mean(abs_xvel)
        abs_yvel = polyvals["Y_vel"].abs()
        y_vel_mean = np.mean(abs_yvel)
        
        abs_xgain = polyvals["X_gain"].abs()
        x_gain_mean = np.median(abs_xgain)
        abs_ygain = polyvals["Y_gain"].abs()
...
class Ui_MainWindow(object):
    ...
    def stim_generator(self):
        ...
        if direct == "Unidirectional" or direct == "Oblique":
            ...
            frame_len = length * framerate
            frame_length = int(frame_len)
            
            linstim_x = [horspeed] * (frame_length)
            linstim_y = [verspeed] * (frame_length)
            
            head_list_x = [float(0)] * int(head*framerate)
            head_list_y = [float(0)] * int(head*framerate)
            tail_list = [float(0)] * int(tail*framerate)
            
            linstim_x.extend(tail_list)
            head_list_x.extend(linstim_x)

            fin_x = head_list_x
                  
            linstim_y.extend(tail_list)
            head_list_y.extend(linstim_y)

            fin_y = head_list_y
           
            linstim_vel = pd.DataFrame()
            linstim_vel["epxWave"] = fin_x
            linstim_vel["epyWave"] = fin_y
            stim_velocity = linstim_vel

        if direct == "Oscillatory":
            amp_rad = amp * (np.pi/180)
            
            head_list = [float(0)] * int(head*framerate)
            tail_list = [float(0)] * int(tail*framerate)
            
            length = numepoch*(lenepoch + lenpoststimedit)
            total_length = numepoch*(lenepoch + lenpoststimedit) + head + tail
            
            frame_length = length * framerate
            frame_total_length = total_length * framerate
            
            freq_frame = freq/framerate
            
            frame_time = np.arange(0,frame_length,1)
            total_frame_time = np.arange(0,frame_total_length,1)
            y=(amp_rad*sin(2*np.pi*freq_frame*x)).diff(x)
            z = lambdify(x,y)
            ...
            final_list = [float(framerate*x/(np.pi/180)) for x in final_list]
            
            df_sine = pd.DataFrame()
            df_sine["Frame"] = total_frame_time
            
            if direction == "Forward (Horizontal)" or direction == "Backward (Horizontal)":
                df_sine["epxWave"] = final_list
                df_sine["epyWave"] = [0] * len(df_sine["epxWave"])
            
            if direction == "Upward (Vertical)" or direction == "Downward (Vertical)":
                df_sine["epyWave"] = final_list
    ...
    def Selector(self):
        ...
        if direc == "Backward (Horizontal)" or direc == "Forward (Horizontal)":
            for x in select_tup:
                ...
                plt.plot("X","epxWave",data=select_fd,color="orange",zorder=2)
        if direc == "Upward (Vertical)" or direc == "Downward (Vertical)":
            for x in select_tup:
                select_fd = fd.loc[x[0]:x[1]]
                plt.plot("X","epyWave",data=select_fd,color="orange",zorder=2)            
    ...
    def retranslateUi(self, MainWindow):
        ...
        self.LinearStim.setText(_translate("MainWindow", "Linear Stimulus"))
        self.OscillatoryStim.setText(_translate("MainWindow", "Oscillatory Stimulus"))
